...
from models.mock_claude import MockClaudeModel
from models.local_llm import OllamaModel, LMStudioModel

# Type for any supported model
ModelType = Union[GeminiModel, OpenAIModel, ClaudeModel, MockClaudeModel, OllamaModel, LMStudioModel]
...
class ModelFactory:
    ...
    @staticmethod
    def get_mock_model_class(model_name: str) -> Type:
        """
        Get the mock model class for a given model name when API key isn't available.
    ...
    def create_model(model_name: str, **kwargs) -> ModelType:
        ...
        if model_family in ["gemini", "openai"]:
            ...
            if api_key:
                # Use the real Claude implementation
                return model_class(model_name=model_name, **kwargs)
            else:
                # If no API key, use the mock implementation
                print("No Anthropic API key found. Using mock Claude implementation.")
                mock_class = ModelFactory.get_mock_model_class(model_name)
                return mock_class(model_name=model_name, **kwargs)
        else:
